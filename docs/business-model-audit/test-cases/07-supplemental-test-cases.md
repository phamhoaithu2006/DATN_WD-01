...
| TC-XD-027 | SR-005 | Avatar file theo UC-019 / BUG-XD-003 | Admin; file JPG hợp lệ | Multipart PUT với field `avatar`; kiểm thêm file + URL và file 5.121 KB | File hợp lệ được lưu public và DB/response có URL; file + URL hoặc quá 5.120 KB trả `422` | `AdminProfileController::update()` | Đã có test: `BusinessModelAuditBugFixTest.php`, ba test avatar admin |
...
| TC-XD-030 | SR-006 | `certificate_type` / BUG-XD-002 | Guide có profile; schema hậu sửa có cột nullable | PUT 100 ký tự, GET lại; PUT 101 ký tự | 100 ký tự được lưu/đọc lại; 101 ký tự trả `422` và không đổi DB | `GuideProfileController::update`; `Guide::$fillable`; migration restore certificate type | Đã có test: `GuideBusinessModelRegressionTest.php`, test round-trip/biên `certificate_type` |
