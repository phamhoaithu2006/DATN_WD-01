...
| --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Authentication, catalog, tour, departure | `docs/business-model-audit/test-cases/01-auth-catalog-test-cases.md` | TC-AC-001–TC-AC-061 | BR-001–BR-020 | 20 | 61 | 8 | 7 | 46 | 0 | 0 |
| Booking, pricing, VNPAY | `docs/business-model-audit/test-cases/02-booking-payment-test-cases.md` | TC-BP-001–TC-BP-065 | BR-021–BR-034 | 14 | 65 | 19 | 16 | 30 | 0 | 0 |
...
| Phân công, điểm danh, nghỉ và thay hướng dẫn viên | `docs/business-model-audit/test-cases/04-guide-operations-test-cases.md` | TC-GOP-001–TC-GOP-073 | BR-054–BR-073 | 20 | 73 | 5 | 0 | 68 | 0 | 0 |
| Support và notification | `docs/business-model-audit/test-cases/05-support-notifications-test-cases.md` | TC-SA-074-01–TC-SA-084-04 | BR-074–BR-084 | 11 | 41 | 0 | 0 | 41 | 0 | 0 |
| Nền tảng quản trị | `docs/business-model-audit/test-cases/06-admin-platform-test-cases.md` | TC-SA-085-01–TC-SA-096-08 | BR-085–BR-096 | 12 | 60 | 0 | 0 | 38 | 0 | 22 |
| Yêu cầu xuyên tài liệu | `docs/business-model-audit/test-cases/07-supplemental-test-cases.md` | TC-XD-001–TC-XD-080 | SR-001–SR-013 | 13 | 80 | 0 | 0 | 73 | 2 | 5 |
| **Tổng** | **7 file** | **437 ID duy nhất** | **BR-001–BR-096; SR-001–SR-013** | **109** | **437** | **57** | **23** | **328** | **2** | **27** |

### Cách đọc trạng thái tự động hóa

- `Đã có test`: cột trạng thái dùng đúng cụm từ này và dẫn test/assertion hiện hữu.
- `Chưa có test`/chưa phủ đủ: cột trạng thái ghi rõ chưa có test hoặc test hiện hữu chưa bao phủ đủ mục tiêu. Nhóm này gồm 7 case của module 01 và 16 case của module 02.
...
- `Có tham chiếu test nhưng không dùng nhãn chuẩn`: 22 case của module 06 dẫn file/test/assertion nhưng không dùng tiền tố `Đã có test`. Trong 22 case này, sáu dòng tự ghi rõ coverage còn thiếu bằng các cụm `chỉ assert` hoặc `chưa cover`: TC-SA-091-01, TC-SA-093-01, TC-SA-095-01, TC-SA-095-04, TC-SA-095-06 và TC-SA-096-07. TC-SA-085-04 có mục tiêu bao gồm download và delete, còn bằng chứng tự động hóa trong dòng chỉ nêu assertion download.
- Năm case supplemental có tham chiếu assertion và không chứa exact missing-evidence phrase là TC-XD-008, TC-XD-054, TC-XD-062, TC-XD-070 và TC-XD-071; chúng được tính vào cột `Có tham chiếu test, không dùng nhãn chuẩn`.
...
- Trạng thái tự động hóa: 8 `Đã có test`; 7 chưa có/chưa bao phủ đầy đủ; 46 `KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE`.
...
- Trạng thái tự động hóa: 19 `Đã có test`; 16 chưa có/chưa bao phủ đầy đủ; 30 `KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE`.
...
- Trạng thái tự động hóa: 5 `Đã có test`; 68 `KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE`.
...
- Trạng thái tự động hóa: cả 41 case đều ghi `KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE`.
...
- Trạng thái tự động hóa: 38 case ghi `KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE`; 22 case dẫn test/assertion hiện hữu mà không dùng nhãn `Đã có test`.
...
- Trạng thái tự động hóa: 73 case chỉ ghi **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE**; 2 case vừa dẫn assertion hiện hữu vừa ghi exact missing-evidence phrase cho phần còn thiếu; 5 case dẫn assertion hiện hữu và không chứa exact missing-evidence phrase.
