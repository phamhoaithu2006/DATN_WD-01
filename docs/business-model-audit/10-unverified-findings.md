...
| UV-026 | Constraint khác nhau giữa SQLite và driver khác | **Bất nhất trực tiếp — còn mở ngoài MySQL/SQLite** | Migrations users/tours/reviews/tour_reviews/partner có nhánh `DB::getDriverName()`; ba migration hậu sửa đã chạy `migrate:fresh`, rollback và migrate lại thành công trên MySQL, đồng thời bộ test SQLite đã chạy; khác biệt tổng quát giữa các driver khác vẫn chưa được xác minh; [ERD](../reverse-engineering/07-database-erd.md), [đối chiếu hậu sửa](11-post-fix-verification.md) |
...
| UV-028 | Notification type `support_message` không nằm trong enum migration | **Đã khép tại hậu sửa** | `SupportNotificationController::sendNotification()` hiện ghi `type = support`, khớp enum migration; `BUG-SA-001` đã **Resolved** và có regression test; [đối chiếu hậu sửa](11-post-fix-verification.md) |
...
| UV-030 | Admin cập nhật payment không có state guard như VNPAY | **Đã khép tại hậu sửa** | `Admin/PaymentController::updateStatus()` hiện khóa bản ghi và áp dụng ma trận `pending -> success|failed`, `failed -> success`, `success -> refunded`; chuyển trạng thái khác trả `422`; `BUG-AB-004` đã **Resolved** và có regression test; [đối chiếu hậu sửa](11-post-fix-verification.md) |
...
| UV-042 | Thiếu test Feature trực tiếp cho nhiều module active | **Còn mở một phần** | Source hiện có 22 file test; bốn suite hậu sửa bao phủ 15 BUG đã đăng ký, gồm sáu kịch bản concurrency MySQL thực. Vẫn **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE** về test trực tiếp cho toàn bộ module active còn lại; mapping tại [SRS](../reverse-engineering/04-srs.md), bảy [test-case artifacts](test-cases) và [đối chiếu hậu sửa](11-post-fix-verification.md) |
| UV-043 | Runtime migration trên từng DB driver | **Đã xác minh MySQL và SQLite; còn mở với driver khác** | 83 migration PHP active; `migrate:fresh`, rollback ba migration hậu sửa và migrate lại đã thành công trên MySQL; bộ test SQLite đã chạy. **KHÔNG TÌM THẤY BẰNG CHỨNG** runtime cho PostgreSQL/SQL Server hoặc driver khác; [đối chiếu hậu sửa](11-post-fix-verification.md) |
