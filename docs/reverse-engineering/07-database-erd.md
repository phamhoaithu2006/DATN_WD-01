...
Kết quả đối chiếu tĩnh hiện hành: **63 bảng duy nhất** từ **83 migration PHP active**, được phủ đủ trong inventory và các sơ đồ ERD bên dưới. Snapshot baseline có 80 migration; ba migration ngày `2026_07_22` chỉ thay đổi cột hiện hữu, không tạo thêm bảng.
...
| `booking_contacts` | `id` PK; `booking_id`; tên liên hệ; `contact_email VARCHAR(150) NULL`; điện thoại; địa chỉ; yêu cầu đặc biệt | `booking_id -> bookings`, D:C/U:C | UQ `booking_id`; IDX `contact_phone` | timestamps | `BookingContact`; `booking(): BelongsTo` | `2026_06_10_220070_create_booking_contacts_table.php::up()`; `2026_07_22_010000_make_booking_contact_email_nullable.php::up()` |
...
| `banners` | `id` PK; title/display title; type; `image_url VARCHAR(500) NULL`; HTML/link; position/pages; sort; hiệu lực; status | Không có FK | IDX position/type/status; IDX `(start_date,end_date)`; enum type `image\|html`; status `active\|inactive`; position/page chỉ là model constants, không phải DB check | timestamps | `Banner`; `scopeVisible()` | `2026_06_10_220190_create_banners_table.php::up()`; `2026_06_13_000002_add_widget_columns_to_banners_table.php::up()`; `2026_07_22_000000_make_banner_image_url_nullable.php::up()` |
...
erDiagram
    ...
    BOOKING_CONTACTS { bigint id PK bigint booking_id FK, UK string contact_email "nullable" }
...
erDiagram
    ...
    GUIDES { bigint id PK bigint user_id FK string guide_code UK string certificate_type "nullable" enum status }
...
erDiagram
    ...
    BANNERS { bigint id PK enum type string image_url "nullable" string position enum status }
...
Các enum chỉ chứng minh tập giá trị, không tự chứng minh transition hợp lệ. DB không có state-machine tổng quát hoặc bảng transition; source hậu sửa có guard cục bộ tại `Admin\PaymentController::updateStatus()` và `Admin\BookingController::update()/softDelete()`, được truy vết tại mục 10.
...
- `2026_07_22_010000_make_booking_contact_email_nullable.php::up()` đổi `booking_contacts.contact_email` thành `VARCHAR(150) NULL`.
- `2026_07_22_000000_restore_certificate_type_to_guides_table.php::up()` thêm lại `guides.certificate_type VARCHAR(100) NULL`.
- `2026_07_22_000000_make_banner_image_url_nullable.php::up()` đổi `banners.image_url` thành `VARCHAR(500) NULL`.
...
- `make_booking_contact_email_nullable::down()` đổi mọi `contact_email IS NULL` thành chuỗi rỗng trước khi khôi phục `VARCHAR(150) NOT NULL`.
- `make_banner_image_url_nullable::down()` đổi mọi `image_url IS NULL` thành chuỗi rỗng trước khi khôi phục `VARCHAR(500) NOT NULL`.
- `restore_certificate_type_to_guides_table::down()` drop cột `certificate_type`; không có backfill dữ liệu vì `up()` chỉ thêm cột nullable mới.
...
## 10. Xác minh hậu sửa ngày 2026-07-22

| Phạm vi | Source hiện hành | Bằng chứng runtime/test |
|---|---|---|
| Ba thay đổi schema | Ba migration `2026_07_22` nêu tại mục 6; `BookingContact`, `Guide`, `Banner` | MySQL 8.4 chạy `migrate:fresh` thành công **83/83**, rollback đúng ba migration mới và migrate lại thành công; SQLite được thực thi qua bộ test dùng `RefreshDatabase` |
| Contact email nullable | `StoreBookingRequest::rules()`; `CustomerBookingController::store()`; migration contact | `AuthBookingBusinessModelRegressionTest.php`: bỏ email vẫn tạo booking/lưu `NULL`; rollback giữ row và đổi null thành chuỗi rỗng |
| Certificate type nullable | `GuideProfileController::show()/update()`; `Guide::$fillable`; migration guide | `GuideBusinessModelRegressionTest.php`: round-trip 100 ký tự; 101 ký tự trả `422` |
| Banner image nullable | `WidgetController::rules()/payload()`; migration banner | `BusinessModelAuditBugFixTest.php`: tạo widget HTML không ảnh; rollback giữ row/nội dung |
| Guard/lock không thuộc schema | `PaymentController::updateStatus()` khóa payment và áp transition; `BookingController::update()/softDelete()` khóa booking; `NotificationController::sendNotification()` khóa draft; `SupportNotificationController::sendNotification()` ghi type `support` | `AuthBookingBusinessModelRegressionTest.php`, `BusinessModelAuditBugFixTest.php`, `BusinessModelConcurrencyMysqlTest.php`; concurrency booking/draft đã chạy đa tiến trình trên MySQL |

Runtime migration đã được xác minh trên MySQL và SQLite theo các artifact trên. **KHÔNG TÌM THẤY BẰNG CHỨNG** runtime cho PostgreSQL, SQL Server hoặc driver khác; khác biệt driver-specific tại mục 8 vì vậy vẫn còn mở ngoài MySQL/SQLite.
