...
### 1.1 Cập nhật hậu sửa ngày 2026-07-22

Tài liệu đã được đối chiếu lại trên working tree nhánh `fix/business-model-audit-bugs`. Source hiện có 83 migration và 22 file test; 15 BUG trong phạm vi audit đã **Resolved**. Kết quả migration, regression và concurrency MySQL được ghi tại [Xác minh hậu sửa Business Model](../business-model-audit/11-post-fix-verification.md). Các FR/NFR bên dưới mô tả contract hiện hành sau sửa.
...
- **Validation:** Register yêu cầu `full_name` tối đa 150, email tối đa 150/unique, phone tối đa 20/unique, password confirmed và đạt `Setting::password_min_length` (mặc định 8); customer profile dùng tên tối đa 150 và phone nullable tối đa 20; avatar customer giới hạn JPG/JPEG/PNG/WebP và 5.120 KB. Admin profile nhận `avatar_url` nullable tối đa 500 hoặc file `avatar` JPG/JPEG/PNG/WebP tối đa 5.120 KB; gửi đồng thời hai nguồn ảnh trả `422`.
...
- **Preconditions:** Customer đã xác thực đúng role; tour/lịch khởi hành tồn tại và phù hợp; khi tạo booking, cấu hình VNPAY phải sẵn sàng; các thao tác customer trên booking yêu cầu quyền sở hữu; xóa cứng phía admin chỉ áp dụng booking đã cancelled. Booking admin đã `cancelled` là terminal và không được chuyển trở lại trạng thái khác.
- **Main Flow:** Preview validate payload và gọi `TourPricingService`; store mở transaction, khóa lịch và tour, định giá từng participant theo DOB, lấy tổng `unit_price` participant làm tổng có thẩm quyền cho cả booking/payment, kiểm tra số chỗ, tạo booking/contact/participants/payment/history, tăng `booked_slots`, rồi trả payment URL; continue-payment tạo/đồng bộ phiên thanh toán phù hợp; cancel chuyển trạng thái và giải phóng chỗ theo service. Update/soft-delete admin khóa booking; chỉ lần chuyển sang `cancelled` đầu tiên giải phóng slot.
- **Alternative Flow:** Lịch có thể dùng giá giảm, giá cơ bản hoặc giá tour theo logic pricing; participant áp dụng rule tuổi và lưu pricing snapshot; `quantity_summary` chỉ được kiểm tính nhất quán, không quyết định tổng tiền lưu; contact email có thể bỏ trống và được lưu `NULL`; customer history trả thêm `can_review_tour` và `tour_review`; continue-payment dùng payment còn hiệu lực hoặc tạo luồng thanh toán theo lifecycle service.
...
- **Validation:** `CustomerBookingController` dùng `StoreBookingRequest` cho tour/departure/contact/participants; preview/store kiểm tra số người, ngày sinh/nhóm tuổi và số chỗ. Contact name/email/phone tối đa 150/150/20, email nullable; participant name/phone tối đa 150/20. Admin controller có validation riêng cho tạo/cập nhật.
...
- **Main Flow:** Controller xác minh chữ ký và dữ liệu callback; lifecycle service khóa payment, xử lý idempotent, cập nhật payment/booking payment status/history; mã thành công ghi nhận thanh toán; mã timeout/hủy/thất bại cập nhật trạng thái và giải phóng chỗ khi logic yêu cầu. Admin confirm/fail/refund mở transaction, khóa payment và chỉ áp dụng ma trận `pending -> success|failed`, `failed -> success`, `success -> refunded`; command tìm payment pending hết hạn để xử lý.
- **Alternative Flow:** `return-status` phục vụ trình duyệt quay lại, `webhooks/vnpay` phục vụ IPN; callback lặp lại không áp dụng side effect lần hai; mã VNPAY `11`/`24` đi vào nhánh hết hạn/hủy được service phân biệt; admin đồng bộ `bookings.payment_status` trong cùng transaction với trạng thái payment.
- **Exception Flow:** Thiếu cấu hình VNPAY trả `503`; chữ ký hoặc số tiền không hợp lệ trả `422`/response code tương ứng của callback; không tìm thấy payment/booking trả `404`; cạnh admin ngoài ma trận, kể cả gửi lại cùng trạng thái, trả `422`.
...
- **Validation:** Xác minh HMAC, terminal/merchant reference, amount và payment state; route admin/status dùng numeric parameter; filter admin được validate trong controller; `updateStatus()` kiểm trạng thái nguồn–đích dưới row lock.
...
- **Preconditions:** Có Sanctum token; endpoint support/admin yêu cầu role tương ứng; notification cá nhân phải thuộc user hiện tại; gửi draft yêu cầu draft tồn tại, còn `status = draft` dưới row lock và xác định được recipient.
- **Main Flow:** Customer/support đọc collection, unread count và chi tiết, rồi mark-as-read; support tạo notification `type = support` cho admin; admin tạo/cập nhật draft, preview recipient, gửi bằng transaction sau khi khóa/tái kiểm tra draft, bulk insert notification rồi đặt draft `sent`; revoke xóa notification theo draft và đưa draft về trạng thái phù hợp; draft có soft delete/restore/force delete; bell API đọc/đánh dấu notification admin.
...
- **Validation:** Create yêu cầu `user_id` unique, experience 0–40, status `active|inactive|locked`, ít nhất một destination distinct; language/level/certificate/year theo rule controller; update dùng `sometimes`; guide self-profile nhận `certificate_type` tối đa 100 ký tự, được model fillable và schema lưu nullable; guide password có validation riêng.
...
- **Preconditions:** Departure có ngày đi trong tương lai theo mutation guard; guide active/có user và không xung đột lịch/leave theo nhánh tương ứng; replacement yêu cầu guide có assignment không cancelled và gửi trước ngày đi ít nhất 5 ngày; admin chỉ xử lý request pending.
- **Main Flow:** Sáu action `candidates`, `autoAssign`, `assign`, `cancel`, `directCandidates`, `directAssign` gọi mutation guard; write flow khóa departure và kiểm tra lại guard. Planning/candidates dựng danh sách đủ điều kiện; auto assign service xếp hạng rồi tạo assignment; assign-specific xác minh và tạo; direct assign luôn chặn lịch/leave và xử lý lệch khu vực theo cờ force; cancel xóa assignment. Guide tạo replacement trong transaction khóa departure/assignment và tái kiểm tra pending; evidence được xóa bù nếu request không được tạo. Approve/reject cùng khóa departure rồi request, tái kiểm tra pending; approve cập nhật assignment/request/notification trong một transaction.
...
- **Exception Flow:** Departure có ngày đi `<= hôm nay` làm cả sáu action assignment/candidate trả `422`; conflict/leave/area mismatch không được force bị từ chối; replacement quá muộn trả `422`; pending request trùng, kể cả dưới cạnh tranh, trả `409`; request đã xử lý trả `409`; không tìm được guide thay thế trả `422`; resource/assignment không khớp trả `404`/authorization error.
...
- **Source Code Reference:** File — `backend_laravel/routes/api.php`, `backend_laravel/app/Http/Controllers/Api/Admin/TourDepartureGuideAssignmentController.php`, `backend_laravel/app/Http/Controllers/Api/Guide/GuideTourController.php`, `backend_laravel/app/Http/Controllers/Api/Admin/AdminGuideReplacementRequestController.php`, `backend_laravel/app/Http/Requests/StoreGuideReplacementRequest.php`, `backend_laravel/app/Services/GuideAssignmentService.php`, `backend_laravel/app/Services/TourDepartureMutationGuard.php`; Class — `TourDepartureGuideAssignmentController`, `GuideTourController`, `AdminGuideReplacementRequestController`, `StoreGuideReplacementRequest`, `GuideAssignmentService`, `TourDepartureMutationGuard`; Method — `planning`, `candidates`, `autoAssign`, `assign`, `cancel`, `directCandidates`, `directAssign`, `requestReplacement`, `replacementRequestStatus`, `index`, `approve`, `reject`, `eligibleGuidesQuery`, `hasScheduleConflict`, `assignSpecific`, `assertCanMutate`; Route — các route nêu tại trường API; Model — `TourGuideAssignment`, `Guide`, `TourDeparture`, `GuideLeaveRequest`; **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE** về model Eloquent riêng cho `guide_replacement_requests`, controller dùng query builder; Migration — `2026_06_28_092905_create_tour_guide_assignments_table.php`, `2026_07_07_080821_add_assignment_fields_to_tour_guide_assignments_table.php`, `2026_07_12_000000_create_guide_replacement_requests_table.php`; Test — `backend_laravel/tests/Feature/GuideBusinessModelRegressionTest.php`, `backend_laravel/tests/Feature/BusinessModelConcurrencyMysqlTest.php`.
...
- **Preconditions:** Guide có profile; bảng leave tồn tại; ngày bắt đầu ít nhất 5 ngày sau; không trùng đơn pending/approved; guide chỉ hủy đơn của mình đang pending; admin không xử lý cancelled hoặc đơn đã hết kỳ theo rule controller.
- **Main Flow:** Guide validate, transaction khóa hàng guide rồi tái kiểm tra overlap bằng locking read trước khi tạo request pending, lưu attachment và notification admin; list/summary chỉ lấy dữ liệu guide hiện tại. Cancel khóa request, tái kiểm tra owner/pending rồi đổi trạng thái/metadata và thông báo. Quyết định admin khóa cùng request, tái kiểm tra cancelled/ngày hết hạn rồi cập nhật approver/note/reviewed time/status và gửi notification trong transaction.
...
- **Exception Flow:** Chưa migrate bảng: list trả collection rỗng kèm message, create trả `500`; thiếu guide profile trả `404`; overlap hoặc trạng thái không cho phép trả `422`; cancel-vs-decision cạnh tranh chỉ một transaction thắng do row lock và bên còn lại trả `422`; sai owner/role bị từ chối; lỗi transaction rollback.
...
- **Source Code Reference:** File — `backend_laravel/routes/api.php`, `backend_laravel/app/Http/Controllers/Api/Guide/GuideLeaveRequestController.php`, `backend_laravel/app/Http/Controllers/Api/Admin/AdminGuideLeaveRequestController.php`; Class — `GuideLeaveRequestController`, `AdminGuideLeaveRequestController`; Method — `index`, `summary`, `store`, `cancel`, `show`, `approve`, `reject`, `updateDecision`, `updateStatus`; Route — các route nêu tại trường API; Model — `GuideLeaveRequest`, `GuideLeaveRequestAttachment`, `Guide`, `Notification`; Migration — `backend_laravel/database/migrations/2026_07_13_000000_create_guide_leave_requests_tables.php`; Test — `backend_laravel/tests/Feature/GuideBusinessModelRegressionTest.php`, `backend_laravel/tests/Feature/BusinessModelConcurrencyMysqlTest.php`.
...
- **Alternative Flow:** Widget type `image` dùng image URL; type `html` dùng HTML content và có thể lưu `image_url = NULL`; start/end date nullable; settings được chia system/security/notification/locale/payment/backup; public endpoint loại các key không nằm trong public allowlist.
...
- **Source Code Reference:** File — `backend_laravel/routes/api.php`, `backend_laravel/app/Http/Controllers/Api/Admin/SettingController.php`, `backend_laravel/app/Http/Controllers/Api/PublicSettingController.php`, `backend_laravel/app/Http/Controllers/Api/Admin/WidgetController.php`, `backend_laravel/app/Http/Controllers/Api/PublicWidgetController.php`; Class — `SettingController`, `PublicSettingController`, `WidgetController`, `PublicWidgetController`; Method — `index`, `update`, `show`, widget `index`/`store`/`show`/`update`/`destroy`/`toggleStatus`; Route — các route nêu tại trường API; Model — `Setting`, `Banner`; Migration — `2026_06_13_000001_create_settings_table.php`, `2026_06_10_220190_create_banners_table.php`, `2026_06_13_000002_add_widget_columns_to_banners_table.php`, `2026_07_22_000000_make_banner_image_url_nullable.php`; Test — `backend_laravel/tests/Feature/BusinessModelAuditBugFixTest.php`.
...
- `DB::transaction()`/transaction tường minh được dùng trong booking, payment, tour review, guide review, support request, gửi notification draft, tour, departure, user/guide, assignment/replacement, attendance/stage và leave request. Hậu sửa bổ sung/khẳng định transaction cho hủy booking admin, tạo/hủy/quyết định leave và cả approve/reject replacement. `NotificationController::revoke()` xóa notification rồi cập nhật draft nhưng không được bọc transaction.
- Source Code Reference đại diện: `CustomerBookingController::store`; `Admin\BookingController::{update,softDelete}`; `VnpayPaymentController::processVnpayResponse`; `Admin\PaymentController::updateStatus`; `Customer\TourReviewController::{store,update}`; `Admin\TourReviewController::updateStatus`; `CustomerSupportRequestController::store`; `GuideLeaveRequestController::{store,cancel}`; `AdminGuideLeaveRequestController::updateStatus`; `GuideTourController::requestReplacement`; `AdminGuideReplacementRequestController::{approve,reject}`; `Admin\NotificationController::sendNotification`; `GuideTourOperationService::{createAttendanceSession,checkIn,checkOut,advanceStage}`.

### NFR-005 — Pessimistic locking tại điểm tranh chấp

- `lockForUpdate()` được dùng trước khi giữ/hoàn slot, chuyển payment, ghi/tính lại review, tạo/hủy assignment, tạo/hủy/quyết định leave, tạo/duyệt/từ chối replacement, hủy booking admin, gửi draft notification, điểm danh và chuyển stage. Write flow assignment còn kiểm tra mutation guard sau khi khóa departure.
- Source Code Reference: các controller/service booking/VNPAY/review hiện có; `Admin/PaymentController.php`; `Admin/BookingController.php`; `Guide/GuideLeaveRequestController.php`; `Admin/AdminGuideLeaveRequestController.php`; `Guide/GuideTourController.php`; `Admin/AdminGuideReplacementRequestController.php`; `Admin/TourDepartureGuideAssignmentController.php`; `Admin/NotificationController.php`; `GuideAssignmentService.php`; `TourDepartureMutationGuard.php`; `GuideTourOperationService.php`. Concurrency test MySQL: `BusinessModelConcurrencyMysqlTest.php`.
...
- `GuideTourController::requestReplacement()` xóa file evidence đã lưu khi transaction không tạo request hoặc phát sinh lỗi.
...
- Ba migration ngày `2026_07_22` có `down()`; hai migration chuyển nullable về NOT NULL chuẩn hóa `NULL` thành chuỗi rỗng trước khi đổi schema. MySQL `migrate:fresh`, rollback ba migration và migrate lại đều đạt.
- Source Code Reference: `backend_laravel/app/Http/Controllers/Api/Admin/LanguageController.php`; `backend_laravel/app/Http/Controllers/Api/Customer/CustomerSupportRequestController.php`; `backend_laravel/app/Http/Controllers/Api/Guide/GuideTourController.php`; migration tour review và ba migration `2026_07_22_*`; [xác minh hậu sửa](../business-model-audit/11-post-fix-verification.md).
...
- Filename backup được allowlist bằng regex; avatar/evidence/attachment có giới hạn MIME và kích thước tại controller. Admin profile nhận file avatar JPG/JPEG/PNG/WebP tối đa 5.120 KB, lưu disk `public/avatars`; gửi đồng thời file và URL bị validation từ chối.
- Source Code Reference: `backend_laravel/config/filesystems.php`; `CustomerSupportRequestController::store`; `GuideLeaveRequestController::store`; `GuideController::{uploadAvatar,deleteAvatar}`; `SupportStaffController::{uploadAvatar,deleteAvatar}`; `AdminProfileController::update`; `DatabaseBackupService`; Test `BusinessModelAuditBugFixTest.php`.
...
| --- | --- | --- |
| FR-001 | `tests/Feature/RbacAuthorizationTest.php`; `ApiRateLimitTest.php`; `BackupSettingsTest.php`; `AuthBookingBusinessModelRegressionTest.php`; `BusinessModelAuditBugFixTest.php` | Login theo status, auth me/role, throttle/password setting; biên validation user/profile và contract admin avatar URL/file. |
...
| FR-004 | `tests/Feature/PaymentBookingSafetyTest.php`; `TourReviewApiTest.php`; `AuthBookingBusinessModelRegressionTest.php`; `BusinessModelAuditBugFixTest.php`; `BusinessModelConcurrencyMysqlTest.php` | Booking/contact/participant/ownership/slot/payment; email nullable, tổng theo DOB, cancelled terminal và hoàn slot đúng một lần kể cả cạnh tranh. |
| FR-005 | `tests/Feature/PaymentBookingSafetyTest.php`; `AuthBookingBusinessModelRegressionTest.php` | IPN/return, HMAC/amount, retry/expiry/idempotency, ownership, đồng bộ và ma trận transition admin payment. |
...
| FR-010 | `tests/Feature/BusinessModelAuditBugFixTest.php`; `BusinessModelConcurrencyMysqlTest.php` | Support notification dùng enum `support`; gửi draft lặp tuần tự/đồng thời chỉ tạo một notification mỗi recipient. Chưa có test trực tiếp cho toàn bộ read/revoke/ownership. |
...
| FR-017 | `tests/Feature/GuideBusinessModelRegressionTest.php` | Guide profile `certificate_type` round-trip, giới hạn 100/101 và schema hậu sửa. Chưa bao phủ toàn bộ CRUD admin guide. |
| FR-018 | `tests/Feature/GuideBusinessModelRegressionTest.php`; `BusinessModelConcurrencyMysqlTest.php` | Guard sáu API assignment, stale-state/replacement và concurrency create/approve-reject. |
...
| FR-020 | `tests/Feature/GuideBusinessModelRegressionTest.php`; `BusinessModelConcurrencyMysqlTest.php` | Overlap/ownership/stale-state; tạo trùng và cancel-vs-approve thực trên MySQL. |
...
| FR-023 | `tests/Feature/BackupSettingsTest.php`; `BusinessModelAuditBugFixTest.php` | Settings/public keys; widget HTML không ảnh, schema nullable và rollback giữ dữ liệu. |
