...
### 2.2. Booking và thanh toán VNPAY phía khách hàng
...
Sơ đồ này không đại diện cho toàn bộ admin API. Admin `PaymentController` có thể ghi `success`, `failed`, `refunded` mà không kiểm tra trạng thái nguồn; admin `BookingController::update` cũng chấp nhận trạng thái booking đích mà không định nghĩa đồ thị chuyển trạng thái.
...
- Test: `backend_laravel/tests/Feature/PaymentBookingSafetyTest.php`; các case tạo payment pending, callback thành công, callback sai chữ ký, thử lại, hủy, timeout và giải phóng chỗ đúng một lần.
...
- Test chuyên biệt cho luồng tạo, hủy và đổi quyết định đơn nghỉ: **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE**.
...
- Test chuyên biệt cho yêu cầu thay hướng dẫn viên: **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE**.
...
- File: `backend_laravel/routes/api.php`; routes: `POST /api/admin/tour-departures/{departure}/auto-assign-guide`, `POST /api/admin/tour-departures/{departure}/assign-guide`, `PATCH /api/admin/tour-departures/{departure}/guide-assignments/{assignment}/cancel`, `POST /api/admin/tour-departures/{departure}/direct-assign-guide`, `POST /api/admin/guide-replacement-requests/{id}/approve`.
...
- File: `backend_laravel/app/Http/Controllers/Api/Admin/TourDepartureGuideAssignmentController.php`; class: `TourDepartureGuideAssignmentController`; methods: `autoAssign`, `assign`, `cancel`, `directAssign`.
...
- Test trực tiếp cho toàn bộ vòng đời assignment: **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE**.
...
- Test chuyên biệt cho gửi và thu hồi campaign: **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE**.
...
### 3.2. Khách đặt tour, VNPAY callback và timeout
...
- File: `backend_laravel/routes/api.php`; route: `POST /api/customer/bookings`, `POST /api/customer/bookings/{booking}/continue-payment`, `PATCH /api/customer/bookings/{booking}/cancel`, `GET /api/webhooks/vnpay`, `GET /api/vnpay/return-status`.
...
- Test: `backend_laravel/tests/Feature/PaymentBookingSafetyTest.php`.
...
sequenceDiagram
    ...
    alt Tự động phân công
        ...
        Service->>DB: Khóa departure và kiểm tra lead cùng điều kiện guide
        Service->>DB: Tạo lead assigned
        ...
        alt Có xung đột lịch hoặc nghỉ
            ...
            Controller->>DB: Transaction lấy lead cũ
            opt Lead cũ khác guide mới
...
- File: `backend_laravel/routes/api.php`; routes: `POST /api/admin/tour-departures/{departure}/auto-assign-guide`, `POST /api/admin/tour-departures/{departure}/assign-guide`, `POST /api/admin/tour-departures/{departure}/direct-assign-guide`, `PATCH /api/admin/tour-departures/{departure}/guide-assignments/{assignment}/cancel`.
- File: `backend_laravel/app/Http/Controllers/Api/Admin/TourDepartureGuideAssignmentController.php`; class: `TourDepartureGuideAssignmentController`; methods: `autoAssign`, `assign`, `directAssign`, `cancel`, `notifyGuideAssigned`, `notifyAdminGuideAutoAssigned`, `notifyAdminGuideDirectAssigned`, `notifyGuideAssignmentRemoved`, `notifyAdminGuideAssignmentCancelled`, `notifyGuideDirectAssigned`, `notifyGuideDirectReplaced`, `notifyAdminGuideReplaced`.
...
- Test chuyên biệt cho auto, specific, direct và hard delete assignment: **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE**.
...
- Migrations: `backend_laravel/database/migrations/2026_07_16_220919_create_support_requests_table.php`, `2026_07_16_220920_create_support_request_attachments_table.php`, `2026_06_24_161627_modify_notifications_table.php`.
- Test chuyên biệt cho luồng này: **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE**.
...
sequenceDiagram
    ...
    GuideAPI->>DB: Kiểm tra pending hoặc approved bị trùng ngày
    alt Bị trùng hoặc validation lỗi
        GuideAPI-->>Guide: HTTP 422
    else Hợp lệ
        GuideAPI->>DB: Transaction tạo đơn pending
        opt Có bằng chứng
    ...
    opt Guide hủy đơn
        ...
        alt Không phải chủ đơn hoặc không pending
            GuideAPI-->>Guide: HTTP 404 hoặc 422
        else Là pending của guide
            GuideAPI->>DB: Transaction đổi cancelled và thông báo admin
...
sequenceDiagram
    ...
    alt Không đủ điều kiện
        ...
        GuideAPI->>DB: Transaction tạo request pending
        GuideAPI->>Notify: Thông báo admin
        GuideAPI-->>Guide: HTTP 201
    end

    Admin->>AdminAPI: POST approve
    AdminAPI->>DB: Tìm request pending và departure
    AdminAPI->>DB: Tìm guide active không trùng lịch ưu tiên ít assignment
    alt Không có guide thay thế
        AdminAPI-->>Admin: HTTP 422
    else Có guide thay thế
        AdminAPI->>DB: Transaction đổi assignment cũ assigned thành cancelled
    ...
    opt Admin từ chối thay vì duyệt
        ...
        AdminAPI->>DB: Chỉ xử lý request pending
        AdminAPI->>DB: Transaction đổi rejected và ghi reviewer
        AdminAPI->>Notify: Báo guide cũ và admin
    end
```
...
sequenceDiagram
    ...
    API->>DB: Transaction tìm draft
    alt Không phải draft
        API-->>Admin: HTTP 404
...
- Migrations: `backend_laravel/database/migrations/2026_06_24_152026_create_notification_drafts_table.php`, `2026_06_24_155228_add_deleted_at_to_notification_drafts_table.php`, `2026_06_24_161627_modify_notifications_table.php`, `2026_06_24_165838_add_draft_id_to_notifications_table.php`.
- Test chuyên biệt cho campaign: **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE**.
...
### 4.2. Booking và kết quả thanh toán VNPAY
...
- File: `backend_laravel/routes/api.php`; route: `POST /api/customer/bookings`, `GET /api/webhooks/vnpay`, `GET /api/vnpay/return-status`, `POST /api/customer/bookings/{booking}/continue-payment`, `PATCH /api/customer/bookings/{booking}/cancel`.
...
```mermaid
flowchart TD
    A([Admin chọn cách phân công]) --> B{Auto specific hay direct}
    B -->|Auto| C[Khóa departure]
    C --> D{Đã có lead assigned hoặc confirmed}
    D -->|Có| E[Trả assignment hiện tại]
    D -->|Không| F[Lọc guide active đủ mọi khu vực không xung đột và có ngày nghỉ]
    F --> G{Có guide phù hợp}
    G -->|Không| X[Trả HTTP 422]
    G -->|Có| H[Tạo lead assigned]
    B -->|Specific| I{Departure đã có lead active}
    I -->|Có| X
    I -->|Không| J{Guide thuộc eligible query và không conflict}
    J -->|Không| X
    J -->|Có| H
    B -->|Direct| K{Trùng lịch hoặc trùng đơn nghỉ}
    K -->|Có| X
    K -->|Không| L{Phù hợp khu vực hoặc đã force}
    L -->|Không| Y[Trả HTTP 409 cần xác nhận]
    L -->|Có| M{Lead cũ khác guide mới}
    M -->|Có| N[Thông báo guide cũ và xóa assignment cũ]
    M -->|Không| O[Giữ assignment hiện tại]
    N --> H
    H --> P[Thông báo guide và admin]
    O --> Q([Trả assignment])
    P --> Q
    E --> Q
```
...
- File: `backend_laravel/routes/api.php`; routes: `auto-assign-guide`, `assign-guide`, `direct-assign-guide`, `guide-assignments/{assignment}/cancel` dưới `/api/admin/tour-departures/{departure}`.
- File: `backend_laravel/app/Http/Controllers/Api/Admin/TourDepartureGuideAssignmentController.php`; class: `TourDepartureGuideAssignmentController`; methods: `autoAssign`, `assign`, `directAssign`, `cancel`.
- File: `backend_laravel/app/Services/GuideAssignmentService.php`; class: `GuideAssignmentService`; methods: `eligibleGuidesQuery`, `applyFairWorkloadOrder`, `hasScheduleConflict`, `autoAssign`, `assignSpecific`.
...
flowchart TD
    ...
    D -->|Có| E{Trùng đơn pending hoặc approved}
    E -->|Có| Y
    E -->|Không| F[Transaction tạo pending attachment và notification]
    F --> G([Trả HTTP 201])
    H([Guide yêu cầu hủy]) --> I{Là chủ đơn và status pending}
    I -->|Không| J[Trả HTTP 404 hoặc 422]
    I -->|Có| K[Transaction đổi cancelled và báo admin]
    L([Admin ra quyết định]) --> M{Đơn cancelled}
    M -->|Có| Y
    M -->|Không| N{End date đã qua}
    N -->|Có| Y
    N -->|Không| O[Đặt approved hoặc rejected]
    O --> P[Ghi admin note reviewer time và notification]
```
...
flowchart TD
    ...
    E --> F[Transaction tạo request pending và báo admin]
    G([Admin xử lý]) --> H{Request còn pending}
    H -->|Không| Z
    H -->|Có| I{Duyệt hay từ chối}
    I -->|Từ chối| J[Transaction đổi rejected và gửi notification]
    I -->|Duyệt| K[Tìm guide active không trùng lịch]
    K --> L{Tìm thấy guide}
    L -->|Không| Y
    L -->|Có| M[Transaction hủy assignment cũ theo status assigned]
    M --> N[Tạo assignment mới assigned]
    N --> O[Đổi request approved và gửi notification]
```
...
flowchart TD
    ...
    D -->|Gửi| F{Bản ghi còn status draft}
    F -->|Không| X[Trả HTTP 404]
    F -->|Có| G[Resolve recipient theo all role specific]
    G --> H{Có recipient}
    H -->|Không| X
    H -->|Có| I[Bulk insert notification unread]
    I --> J[Đổi campaign sent]
    J --> K{Admin thu hồi}
    K -->|Không| L([Giữ sent])
    K -->|Có| M{Campaign còn sent}
    M -->|Không| X
    M -->|Có| N[Xóa notification theo draft id]
    N --> O[Đổi campaign về draft]
```
...
- `PaymentController` của admin và `BookingController::update` không có ma trận trạng thái nguồn sang đích; vì vậy chúng không được ghép vào state machine VNPAY có guard.
...
- Guard backend chặn phân công khi lịch đã bắt đầu trong các method của `TourDepartureGuideAssignmentController`: **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE**; các method này không gọi `TourDepartureMutationGuard`.
...
- Test chuyên biệt cho support status, leave decision, replacement approval, notification campaign, chatbot fallback, assignment lifecycle và stage advance: **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE**. Các sơ đồ tương ứng dựa trên controller/service/migration, không được trình bày như hành vi đã được test tự động.

**Source Code Reference**

- Booking/payment: `backend_laravel/app/Http/Controllers/Api/Customer/VnpayPaymentController.php`, methods `processVnpayResponse`; `backend_laravel/app/Http/Controllers/Api/Admin/PaymentController.php`, methods `confirm`, `fail`, `refund`; `backend_laravel/app/Http/Controllers/Api/Admin/BookingController.php`, method `update`; models `App\Models\Booking`, `Payment`; migrations `2026_06_10_220060_create_bookings_table.php`, `2026_06_10_220090_create_payments_table.php`; route tại `backend_laravel/routes/api.php`.
- Support/tour/departure: `backend_laravel/app/Http/Controllers/Api/Support/SupportRequestController.php::updateStatus`, `backend_laravel/app/Http/Controllers/Api/Admin/TourManagerController.php::update/hide/unhide`, `backend_laravel/app/Http/Controllers/Api/Admin/TourDepartureController.php::update`; models `SupportRequest`, `Tour`, `TourDeparture`; migrations `2026_07_16_220919_create_support_requests_table.php`, `2026_06_10_220020_create_tours_table.php`, `2026_06_10_220040_create_tour_departures_table.php`; routes tại `backend_laravel/routes/api.php`.
- Attendance/stage/assignment: `backend_laravel/app/Services/GuideTourOperationService.php`, methods `assertSessionCanTakeAttendance`, `advanceStage`; `backend_laravel/app/Http/Controllers/Api/Admin/TourDepartureGuideAssignmentController.php`, methods `autoAssign`, `assign`, `directAssign`, `cancel`; `backend_laravel/app/Services/TourDepartureMutationGuard.php`, method `assertCanMutate`; models `AttendanceSession`, `TourDepartureStage`, `TourGuideAssignment`; migrations `2026_07_02_143241_create_guide_attendance_and_stage_tables.php`, `2026_06_28_092905_create_tour_guide_assignments_table.php`; routes tại `backend_laravel/routes/api.php`.
- Replacement: `backend_laravel/app/Http/Controllers/Api/Guide/GuideTourController.php::requestReplacement`, `backend_laravel/app/Http/Controllers/Api/Admin/AdminGuideReplacementRequestController.php::approve/reject`; migration `backend_laravel/database/migrations/2026_07_12_000000_create_guide_replacement_requests_table.php`; routes tại `backend_laravel/routes/api.php`; model riêng: **KHÔNG TÌM THẤY BẰNG CHỨNG TRONG SOURCE CODE**.
