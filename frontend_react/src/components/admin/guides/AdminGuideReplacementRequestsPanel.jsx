...
export default function AdminGuideReplacementRequestsPanel({ open = true, highlightRequestId = '', initialTab = 'pending', onClose }) {
  ...
  return <section className="admin-guide-leave-panel">
    ...
    <div className="admin-guide-leave-filter compact"><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Tìm tour hoặc hướng dẫn viên..." /><button type="button" onClick={() => setSearch('')}>Đặt lại</button><button type="button" onClick={() => setActiveTab('history')}>Timeline</button></div>
