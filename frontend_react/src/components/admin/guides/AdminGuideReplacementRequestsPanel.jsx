...
export default function AdminGuideReplacementRequestsPanel({ open = true, highlightRequestId = '', initialTab = 'pending', onClose, showHeader = true }) {
  ...
  useEffect(() => {
    if (['pending', 'approved', 'history'].includes(initialTab)) setActiveTab(initialTab)
  }, [initialTab])
  ...
  return <section className="admin-guide-leave-panel">
    {showHeader ? <div className="admin-guide-leave-head"><div><span>Đơn đổi HDV</span><h3>Quản lý đơn đổi HDV</h3><p>Chấp nhận hoặc không chấp nhận yêu cầu đổi hướng dẫn viên.</p></div>{onClose ? <button type="button" onClick={onClose}>Đóng</button> : null}</div> : null}
    {error ? <div className="admin-guide-leave-alert error">{error}</div> : null}
    <div className="admin-guide-leave-tabs" role="tablist" aria-label="Lọc đơn đổi HDV">{TABS.map((tab) => <button key={tab.key} type="button" className={`${activeTab === tab.key ? 'active ' : ''}${tab.key === 'history' ? 'timeline-tab' : ''}`} onClick={() => setActiveTab(tab.key)}>{tab.label}{tab.key === 'pending' && Number(summary.pending_count || 0) > 0 ? <span>{summary.pending_count}</span> : null}</button>)}</div>
