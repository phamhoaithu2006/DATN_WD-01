...
const TourDepartureTable = ({ departures = [], selectedTourId, onDelete }) => {
  ...
  const getRemainSlotClass = (remainSlots) => {
    if (remainSlots <= 0) return "text-rose-600 bg-rose-50 ring-rose-100";
    if (remainSlots <= 5) return "text-amber-600 bg-amber-50 ring-amber-100";
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-slate-500">
              Quản lý ngày đi, ngày về, số chỗ và trạng thái của từng lịch.
            </p>
          </div>
        </div>
            ...
            <table className="w-full min-w-[1050px] border-collapse text-sm">
              <thead>
                <tr className="bg-slate-50 text-center text-[12px] font-black uppercase tracking-[0.04em] text-slate-500">
                  ...
                  <th className="border-b border-slate-200 px-4 py-4">Tổng chỗ</th>
                  <th className="border-b border-slate-200 px-4 py-4">Đã đặt</th>
                  <th className="border-b border-slate-200 px-4 py-4">Còn lại</th>
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {departures.length === 0 ? (
                  ...
                  departures.map((item, index) => {
                    ...
                    const remainSlots = Math.max(totalSlots - bookedSlots, 0);
                    const statusMeta = getStatusMeta(item.status);

                    return (
                      <tr
                        key={item.id}
                        className="text-slate-700 transition duration-150 hover:bg-sky-50/40"
                      >
                        <td className="px-4 py-4 text-center">
                          <span className="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-slate-50 text-xs font-black text-slate-600 ring-1 ring-slate-200">
                            {index + 1}
                          </span>
                        </td>
