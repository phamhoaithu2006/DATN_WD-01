...
function DestinationPlaceManagementPage() {
  ...
  const fetchPlaces = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await destinationPlaceApi.getAll({
        ...
        activity_type: appliedActivity || undefined,
  ...
  }, [provinceId, appliedActivity, appliedSearch, appliedStatus])
  ...
  return (
    <div className="min-h-full bg-[#f8fafc] px-4 py-6 sm:px-6 lg:px-8">
      <AdminPageHeader
        breadcrumb={['ViVuGo', 'Quản lý Tour', 'Địa điểm theo tỉnh']}
        title="Địa điểm theo tỉnh"
        description="Quản lý danh sách địa điểm và hoạt động du lịch theo từng tỉnh/thành phố."
        actions={
          <Link
            to="/admin/tours"
            className="inline-flex h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-xs font-bold text-slate-700 shadow-2xs transition hover:border-sky-300 hover:bg-sky-50/50 hover:text-sky-700 focus-visible:outline-2 focus-visible:outline-sky-600"
          >
        ...
        <main className="space-y-5 lg:col-span-8 xl:col-span-9">
          ...
          <div className="flex flex-col gap-4 rounded-2xl border border-slate-200/80 bg-white p-5 shadow-2xs sm:flex-row sm:items-center sm:justify-between">
            ...
            <div>
              {provinceId ? (
                <Link
                  to={`/admin/destination-places/create?province_id=${provinceId}`}
                ...
                {places.map((place) => (
                  ...
                  >
                          ...
                          >
                            <span
                              className={`h-1.5 w-1.5 rounded-full ${
                                place.status === 'active' ? 'bg-emerald-500' : 'bg-slate-400'
                              }`}
                            />
                            {place.status === 'active' ? 'Đang hoạt động' : 'Tạm ẩn'}
                          </span>
                        </div>
                    ...
                    <div className="mt-4 flex items-center justify-end gap-2 border-t border-slate-100 pt-3 sm:mt-0 sm:border-0 sm:pt-0 sm:pl-3">
                      <Link
                        to={`/admin/destination-places/${place.id}/edit`}
            ...
            {provinceId && pagination.last > 1 && (
              <div className="flex items-center justify-between rounded-2xl border border-slate-200/80 bg-white px-5 py-3.5 shadow-2xs">
                <p className="text-xs font-semibold text-slate-500">
                  Hiển thị <span className="font-bold text-slate-800">{places.length}</span> / <span className="font-bold text-slate-800">{pagination.total}</span> địa điểm
                </p>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    ...
                    className="inline-flex h-9 items-center gap-1 rounded-lg border border-slate-200 px-3 text-xs font-bold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    ← Trước
                  </button>

                  <span className="px-2 text-xs font-extrabold text-slate-700">
