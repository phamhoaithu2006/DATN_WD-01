...
class CustomerController extends Controller
{
    ...
    {
        // 1. Xác thực đầu vào
        // Yêu cầu người dùng cung cấp thông tin định danh (email hoặc số điện thoại)
        $request->validate(['identifier' => 'required']); 

        // 2. Tìm kiếm người dùng
        // Tìm bản ghi trong DB khớp với email HOẶC số điện thoại
        $user = User::where('email', $request->identifier)
                    ->orWhere('phone', $request->identifier)
                    ->first();

        // 3. Xử lý trường hợp không tìm thấy
        // Nếu không tồn tại, trả về mã 404 (Not Found) để bảo mật, 
        // tránh tiết lộ thông tin tài khoản có tồn tại hay không
        if (!$user) return response()->json(['message' => 'Tài khoản không tồn tại'], 404);

        // 4. Tạo và lưu mã OTP
        // Tạo số ngẫu nhiên 6 chữ số và lưu vào cột 'otp' trong bảng users
        $otp = rand(100000, 999999);
        $user->update(['otp' => $otp]);

        // 5. Gửi thông báo
        // Tại đây bạn sẽ tích hợp dịch vụ gửi Email (ví dụ: Mail::to($user->email)->send(...))
        // hoặc SMS API để gửi $otp cho người dùng
