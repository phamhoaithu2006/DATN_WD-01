...
{
    // Kiểm tra thủ công: Nếu không có user (tức là chưa đăng nhập)
    if (!$request->user()) {
        return response()->json([
            'status' => 'error',
            'message' => 'Bạn chưa đăng nhập, vui lòng đăng nhập để thực hiện tính năng này'
        ], 401); // Trả về 401 thay vì 500
    }

    // Nếu đã đăng nhập, tiến hành validate và lưu
