...
class CustomerManagerController extends Controller
{
    ...
    {
        // Tạo một khung truy vấn cơ sở cho bảng User, lọc theo role_id = 2 (thường là Khách hàng)
        // Việc dùng biến này giúp tránh phải viết lại điều kiện where('role_id', 2) nhiều lần
        $baseQuery = User::where('role_id', 2);

        // Dùng 'clone' để tạo bản sao của khung truy vấn, tránh thay đổi trạng thái của biến gốc $baseQuery
        // Đếm tổng số người dùng có role_id là 2
        $total = (clone $baseQuery)->count();

        // Lọc thêm điều kiện 'status = active' trên bản sao của khung truy vấn để đếm số user đang hoạt động
        $active = (clone $baseQuery)->where('status', 'active')->count();

        // Tương tự, lọc 'status = inactive' để đếm số user đang bị khóa/không hoạt động
        $locked = (clone $baseQuery)->where('status', 'inactive')->count();

        // Truy vấn từ bảng Booking, sử dụng whereHas để lọc các đơn đặt lịch 
        // của những người dùng có role_id = 2
        $totalBookings = Booking::whereHas('user', function ($query) {
            $query->where('role_id', 2);
        })->count();

        // Trả về kết quả dưới dạng JSON cho phía Client (Frontend) sử dụng
        return response()->json([
            'status' => 'success',
            'data' => [
                'total'          => $total,          // Tổng số user
                'active'         => $active,         // Số user đang hoạt động
                'locked'         => $locked,         // Số user đang bị khóa
                'total_bookings' => $totalBookings,  // Tổng số lượt đặt lịch của nhóm user này
            ],
        ], 200);
    }
