...
{
    // 1. Tìm bản nháp để xác nhận nó đã được gửi
    $draft = \App\Models\NotificationDraft::where('id', $draft_id)
                ->where('status', 'sent')
                ->first();

    if (!$draft) {
        return response()->json(['message' => 'Không tìm thấy chiến dịch thông báo đã gửi!'], 404);
    }

    // 2. Xóa tất cả các thông báo đã gửi cho user trong bảng notifications
    // Nếu bạn có cột 'draft_id' trong bảng notifications:
    $deletedCount = \App\Models\Notification::where('draft_id', $draft_id)->delete();

    // 3. (Tùy chọn) Cập nhật lại trạng thái bản nháp nếu muốn cho phép gửi lại
    $draft->update(['status' => 'draft']);

    return response()->json([
        'message' => "Đã thu hồi thành công $deletedCount thông báo đã gửi.",
    ], 200);
}
}
