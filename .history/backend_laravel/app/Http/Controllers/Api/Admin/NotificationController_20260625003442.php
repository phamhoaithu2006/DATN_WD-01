...
class NotificationController extends Controller
{
    ...
    public function sendNotification($id)
    {
        ...
        return DB::transaction(function () use ($id) {
            ...
            // Chỉ lấy ID của user để tiết kiệm RAM
            $users = match ($draft->target_type) {
                'all'      => User::select('id')->get(),
                'role'     => User::whereIn('role_id', $draft->target_ids)->select('id')->get(),
                'specific' => User::whereIn('id', $draft->target_ids)->select('id')->get(),
                default    => collect(),
            };

            if ($users->isEmpty()) {
                return response()->json(['message' => 'Không tìm thấy người nhận phù hợp!'], 404);
            }

            // Chuẩn bị mảng dữ liệu để insert 1 lần duy nhất (Bulk Insert)
            $notifications = $users->map(function ($user) use ($draft) {
                return [
                    'draft_id'   => $draft->id,
                    'user_id'    => $user->id,
                    'title'      => $draft->title,
    ...
    public function getUsers(Request $request)
    {
        ...
        // Lọc theo danh sách Role (ví dụ: gửi mảng role_ids [1, 3])
    ...
    //Hiển thị danh sách user đã đc chọn (XIII)
    public function previewRecipients(Request $request)
    {
        $users = User::whereIn('id', $request->input('user_id', []))
            ->orWhereIn('role_id', $request->input('role_ids', []))
            ->select('id', 'full_name')
            ->distinct() // Loại bỏ trùng lặp nếu user thuộc cả 2 điều kiện
            ->get();
