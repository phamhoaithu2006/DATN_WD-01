...
class GuideSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Tiếng Anh dùng A1-C2
        $engC2 = LanguageLevel::where('language_id', $english->id)->where('level_name', 'C2')->first();
        $engC1 = LanguageLevel::where('language_id', $english->id)->where('level_name', 'C1')->first();

        // Tiếng Pháp dùng A1-C2
        $freB2 = LanguageLevel::where('language_id', $french->id)->where('level_name', 'B2')->first();
        $freC1 = LanguageLevel::where('language_id', $french->id)->where('level_name', 'C1')->first();

        // Tiếng Trung dùng HSK1-HSK6
        $chiHsk4 = LanguageLevel::where('language_id', $chinese->id)->where('level_name', 'HSK4')->first();
        ...
        $guidesData = [
            [
                'user_id'            => $userHung->id,
                'guide_code'         => 'HDV001',
                'specialization_ids' => [$specQuocTe->id, $specNoiDia->id],
                'experience_years'   => 8,
                'average_rating'     => 4.90,
                'review_count'       => 42,
                'status'             => 'active',
                'languages' => [
                    ['language_id' => $english->id, 'level_id' => $engC2->id],
                    ['language_id' => $french->id,  'level_id' => $freB2->id],
                ],
                'experiences' => [
            ...
            [
                ...
                'languages' => [
                    ...
                    ['language_id' => $chinese->id, 'level_id' => $chiHsk4->id],
                ],
                'experiences' => [],
            ],
            [
                'user_id'            => $userDuc->id,
                'guide_code'         => 'HDV003',
                'specialization_ids' => [$specQuocTe->id],
                'experience_years'   => 10,
                'average_rating'     => 5.00,
                'review_count'       => 56,
                'status'             => 'active',
                'languages' => [
                    ['language_id' => $english->id, 'level_id' => $engC2->id],
                    ['language_id' => $french->id,  'level_id' => $freC1->id],
                ],
                'experiences' => [
